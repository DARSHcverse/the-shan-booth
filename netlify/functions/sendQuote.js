...
exports.handler = async (event) => {
  ...
  const {
    ...
    message,
    price,
    invoiceNumber, // <-- New field from booking form
  } = data;
  ...
  // ----------------------
    ...
    try {
      ...
      // 2. Create Invoice Item with description including invoice number
      await stripe.invoiceItems.create({
        customer: customer.id,
        amount: price * 100, // AUD to cents
        currency: 'aud',
        description: `${packageDuration} - ${service} (Invoice: ${invoiceNumber})`,
      });

      // 3. Create Invoice with metadata for invoice number
      ...
      // Include invoice link in customer email
      const customerMsg = {
        to: email,
      ...
      // Return invoice URL to frontend for optional redirect
      return {
        statusCode: 200,
